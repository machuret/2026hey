    ...
    const crmRows = jobs.map((j: Record<string, unknown>) => {
      ...
      return {
        ...
        notes:          [
          ...
          j.ai_role_seniority ? `Role: ${j.ai_role_seniority} ${j.ai_role_function || ""}`.trim() : null,
          j.ai_employment_type ? `Employment: ${j.ai_employment_type}` : null,
          j.ai_urgency ? `Urgency: ${j.ai_urgency}${j.ai_urgency_clues ? ` — ${j.ai_urgency_clues}` : ""}` : null,
          j.ai_candidate_persona ? `Candidate persona: ${j.ai_candidate_persona}` : null,
          j.ai_pitch_angle ? `Pitch: ${j.ai_pitch_angle}` : null,
          j.ai_email_snippet ? `Email snippet: ${j.ai_email_snippet}` : null,
          j.ai_objection_preempt ? `Objection: ${j.ai_objection_preempt}` : null,
          j.ai_required_skills ? `Skills: ${Array.isArray(j.ai_required_skills) ? (j.ai_required_skills as string[]).join(", ") : j.ai_required_skills}` : null,
